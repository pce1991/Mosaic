...
@TODO: rendering idea to use textures for the tiles
- depth to determine the order
- with heavy blending the edges are always going to have different traits/feels than the body of the image. 
- possible we allow two images per-tile. But why not more? layers of grids
- wonder about giving a cluster of tiles a single image to draw. Fits the idea of layered grids.


@TODO: image editor where you specify color/sprite per cell
- need to save file to disk

@TODO: audio sequencer
- write in text file? 
- editor piano roll. N voices. Click a voice to bring up piano roll, input.
  - horz scroll regions. 
