...
void MyMosaicInit() {
    ...
    state.boxes = PushSize(&arena, Box, 6);
    state.boxCount = 2;
    ...
    state.undoRecords = PushSize(&arena, UndoRecord, state.undoRecordCapacity);
}
...
void MyMosaicUpdate() {
    ...
    if (InputPressed(Input, Input_Space)) {
        ...
        record.boxPositions = PushSize(&undoArena, vec2i, state.boxCount);
...
void MyMosaicInit() {
    ...
    positions = PushSize(&arena, vec2, positionCount);

    particleSystem.capacity = 5000;
    particleSystem.count = 0;
    particleSystem.particles = PushSize(&arena, ParticleState, particleSystem.capacity);

    particleSystem.spawnRate = 0.05f;
    particleSystem.particlesPerSpawn = 10;

    particleSystem.chainLength = 5;
    particleSystem.velocityChain = PushSize(&arena, vec2, particleSystem.chainLength);
